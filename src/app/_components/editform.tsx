...
const postSchema = z.object({
    ...
    slug: z.string().min(1, "Slug 不能为空").regex(/^[a-z0-9-]+$/, "Slug 仅允许小写字母、数字、连字符"),
...
export default function EditForm({ post }: { post: any }) {
    ...
    useEffect(() => {
        ...
        setTagIds(post.tags?.map((tag: any) => tag.id) ?? []);
        setBody(post?.body ?? '');
        fetchCategories();
        fetchTags();
    }, [post?.categoryId, post?.tags, post?.body]);
    // 处理数据请求
    ...
    const fieldClass =
        "w-full rounded-lg border border-[var(--border)] bg-[var(--card)] px-3 py-2 text-[var(--foreground)] placeholder:text-[var(--muted)] focus:outline-none focus:ring-2 focus:ring-[var(--accent)] focus:border-[var(--accent)] transition-colors";
    ...
    return (
        <div className="max-w-3xl">
            <form onSubmit={handleSubmit} className="form-page space-y-5">
                ...
                <div>
                    <label htmlFor="tagIds" className={labelClass}>
                        标签
                    </label>
                    <Select
                        mode="multiple"
